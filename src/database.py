...
class Database():
    ...
    def add_user(self, screen_name, friends, followers, followed, followed_at, reciprocal, unfollowed, unfollowed_at, update_at):
        """
        >>> db = Database(tablename='test_twitter_users', drop_anyway=True)
        >>> db.add_user('test_user1', 0, 0, False,'', False, False, '', '')
        >>> db.count_users()
        [(1,)]
        """
        self.cursor.execute(
            """INSERT OR IGNORE INTO {} 
            (screen_name, friends, followers, followed, followed_at, reciprocal, unfollowed, unfollowed_at, update_at) 
    ...
    def read_users(self, count=10):
        """
        >>> db = Database(tablename='test_twitter_users', drop_anyway=True)
        >>> db.add_user('u1', 0, 0, False, '', False, False, '', '')
        >>> db.add_user('u2', 9, 1, False, '', False, False, '', '')
        >>> db.add_user('u3', 3, 1, False, '', False, False, '', '')
        >>> db.read_users()
    ...
    def follow_count_today(self):
        """
        >>> db = Database(tablename='test_twitter_users', drop_anyway=True)
        >>> db.add_user('u1', 0, 0, True, '2019-12-07 09:23:13', False, False, '', '')
        >>> db.add_user('u2', 9, 1, True, '2019-12-09 09:23:13', False, False, '', '')
        >>> db.add_user('u3', 3, 1, True, datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"), False, False, '', '')
    ...
    def unfollow_count_today(self):
        """
        >>> db = Database(tablename='test_twitter_users', drop_anyway=True)
        >>> db.add_user('u1', 0, 0, False, '', False, False, '2019-12-07 09:23:13', '')
        >>> db.add_user('u2', 9, 1, False, '', False, False, '2019-12-08 09:23:13', '')
        >>> db.add_user('u3', 3, 1, False, '', False, False, '2019-12-09 09:23:13', '')
